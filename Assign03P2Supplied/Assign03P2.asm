...
#################
                                        addiu $sp, $sp, -109
                                        j StrInitCode           # clutter-reduction 
                                                                # jump (string initialization)
endStrInit:                                     
#   do
#   {
begWBodyM1:
#      valsToDo = GetOneIntByVal(vtdPrompt);
                                        ...
                                        li $v0, 11
                                        syscall                 # '\n' to offset 
                                                                # effects of syscall #12 drawback
                                        addi $a0, $sp, 28
                                        jal GetOneIntByVal
                                        sw $v0, 77($sp)
...
#            GetOneIntByAddr(intArr + valsToDo - i, entIntPrompt);
                                        ...
                                        add $a0, $a1, $a2       # add intArr address with 
                                                                #value and load into $a1 for function call
                                        addi $a1, $sp, 41       # load address of 
                                                                # entIntPrompt into $a0 for function
...
####################(3)####################
                                        li $t1, 0               #load 0 to i
                                        lw $t2, 77($sp)         # load valsToDo into j
                                        addi $t2, $t2, -1       # subtract 1 from j
                                        j FTestM2
begFBodyM2:
...
####################(5)####################
                                        move $a0, $t1
                                        sll $a0, $a0, 2         # multiply i by 4 for indexing
                                        move $a1, $t2
                                        sll $a1, $a1, 2         # multiply i by 4 for indexing
                                        add $a0, $a0, $sp       # load intArr into $a0
                                        addi $a0, $a0, 81
                                        add $a1, $a1, $sp       # load intArr into $a1
                                        addi $a1, $a1, 81       # load intArr into $a1
                                        jal SwapTwoInts
...
####################(3)####################
                                        addi $a0, $sp, 81       # load intArr
                                        lw $a1, 77($sp)         # load valsToDo
                                        addi $a2, $sp, 66       # load flipLab
...
####################(2)####################
                                        addi $a0, $sp, 76       # load address for reply
                                        addi $a1, $sp, 56       # load address dmPrompt
                                        jal GetOneCharByAddr
#   }
#   while (reply != 'n' && reply != 'N');

####################(1)####################
                                        addi $v1, $sp, 76       # move reply into $t0
                                        lb $v1, 0($v1)
                                        li $t0, 'n'
...
GetOneIntByAddr:
#   cout << prompt;
                                        move $t0, $a0           # $t0 has saved 
                                                                # copy of $a0 as received
                                        move $a0, $a1
                                        li $v0, 4
                                        syscall
...
####################(4)####################
                                        ...
                                        lw $t0, 0($a0)          # load *intPtr1 into $t0
                                        lw $t3, 0($a1)          # load *intPtr2 into $t3
                                        sw $t3, 0($a0)          # store *intPtr2 in *intPtr1 
                                        sw $t0, 0($a1)          # store temp in *intPtr2
...
####################(7)####################
                                        ...
                                        move $a0, $a1           # move $a1 into $a0 for syscall
                                        ...
                                        sb $v0, 0($t0)          # store character in charVarToPutInPtr
